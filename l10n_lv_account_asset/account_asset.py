...
class account_asset_category(osv.osv):
    ...
    _columns = {
        'next_month': fields.boolean('Compute from Next Month',
                                     help='Indicates that the first '
                                     ' depreciation entry for this asset has to'
                                     ' be done from the start of the next month'
                                     ' following the month of the purchase '
                                     ' date.\n'
                                     'This cannot be use in combination with'
                                     ' prorata check.'
                                     ),
    }
...
class account_asset_asset(osv.osv):
    ...
    def compute_depreciation_board(self, cr, uid, ids, context=None):
        ...
        for asset in self.browse(cr, uid, ids, context=context):
            ...
            # Logic to create depreciation table
            amount_to_depr = asset.purchase_value
            if len(posted_depreciation_line_ids) > 0:
                last_depreciation_line = depreciation_lin_obj.browse(cr, uid, posted_depreciation_line_ids[0], context=context)
                residual_amount = last_depreciation_line.remaining_value
            else:
            ...
            for x in range(len(posted_depreciation_line_ids), undone_dotation_number):
                i = x + 1
                # increment date if there are already posted entries
                if x > 0:
    ...
    def validate(self, cr, uid, ids, context=None):
        """
        Validate an asset assigning a confirmed date and
        calculating depreciation board if not available
        """
        if context is None:
            context = {}
        for asset in self.browse(cr, uid, ids, context=context):
            confirmation_date = time.strftime('%Y-%m-%d')
            if asset.confirmation_date:
                confirmation_date = asset.confirmation_date
            self.write(cr, uid, asset.id,
                        {'state':'open',
                         'confirmation_date':confirmation_date,
                        }, context)
        return self.compute_depreciation_board(cr, uid, ids, context=context)
    ...
    _columns = {
        ...
        'confirmation_date': fields.date('Date Confirmed', readonly=True, states={'draft':[('readonly', False)]}),
...
class account_asset_asset_depreciation_line(osv.osv):
    ...
    _columns = {
        'asset_value': fields.related('asset_id', 'purchase_value',
                                      type='float', string='Asset Value'),
    ...
    _defaults = {
         'asset_id': lambda self, cr, uid, context: context.get('active_id', False),
         'depreciated_value': 0,
         'remaining_value': 0,
    }
    ...
    def onchange_amount(self, cr, uid, ids, asset_value, amount, depreciated_value, context=None):
        res = {}
        res['value'] = {'remaining_value': (asset_value -
    ...
    def create(self, cr, uid, vals, context=None):
        if not context:
            context = {}
        asset_line_id = super(account_asset_asset_depreciation_line, self).create(cr, uid, vals, context=context)
        line = self.browse(cr, uid, [asset_line_id], context)[0]
        values = {
            'remaining_value': (line.asset_value -
                                line.depreciated_value - line.amount)
        }
        self.write(cr, uid, [asset_line_id], values, context)
